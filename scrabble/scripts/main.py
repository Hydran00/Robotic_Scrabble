...
def move():
    ...
    for i, strr in enumerate(possArray):
        if costFunc(strr, possStart[i][0], possStart[i][1], possStart[i][2]) > mx:
            mx = costFunc(strr, possStart[i][0],
                          possStart[i][1], possStart[i][2])
            ansIndex = i
    ...
    if ansIndex == -1:
        ...
        return "",0,0,0
    getboardCopy()
    #backup last board state for future check
    with open(path+'board.txt', 'r') as board_t, open(path+'old_board.txt','w+') as old_board_t:      
        for line in board_t:
    ...
    print("Best Posible String is " + possArray[ansIndex] + ". Starting is at row " + str(
        possStart[ansIndex][0]+1)+" and col at " + str(possStart[ansIndex][1]+1)+" and point is "+str(mx))
...
def userMove():
    ...
    # input('Enter column of your word starting point\n>')
    co = new_word.column
    row = int(ro)
    col = int(co)
    x = row-1
...
def changeRack(which, positions, rack=""):
    ...
    if which % 2 == 0:
        ...
        if(rack == ""):
            ...
            for i in range(0, 4):
                rack += random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G',
                'H', 'I', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'Z'])
        else:
            for i in range(0, 7):
                if(rack[i] == ' '):
                    rack = rack.replace(' ', random.choice(['A', 'B', 'C', 'D', 'E', 'F', 'G',
                'H', 'I', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'Z']), 1)
        ...
        for j, c in enumerate(s):
            ...
            if((i+1) == row and (j+1)==col):
                if s[j] != "#":
                    ...
                    print("Lettera ",s[j],"in rc",row," ",col," gia presente sulla scacchiera")
                    return True
                else:
                    
                    print("Lettera nuova in ",row," ",col," : ",s[j])
                    return False
    return False
...
if __name__ == "__main__":
    ...
    pub1 = rospy.Publisher(
        "scrabble/board_state_update_request", Bool, queue_size=1)
    f = open(path+'board.txt', 'w')
    for i in range(0, 17):
        if i == 16:
            f.write("#################")
    ...
    while 1:
        ...
        print("\nComputer Score : " + str(cscore) + "\t\t\t" +
              str(userName)+" Score :"+str(userScore)+"\n")
        ...
        if movecnt % 2 == 0:
            ...
            if (word != ""):
                ...
                for x in word:
                    rack_counter = 1
                    for y in rrack:
                        if (y == x and x != ' ' and not letter_already_on_board(row_counter,col_counter)):
